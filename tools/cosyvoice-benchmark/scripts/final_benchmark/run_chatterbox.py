...
def run_chatterbox(output_dir: Path, ref_audio: Path):
    ...
    warmup_pcm = (
        (np.asarray(warmup_samples, dtype=np.float32).reshape(-1) * 32767.0)
        .astype(np.int16)
        .tobytes()
    )
        ...
        for trial in range(TRIALS):
            ...
            try:
                ...
                gen_result = model.generate(
                    sentence["text"], audio_prompt_path=str(ref_audio)
                )
                generation_time = time.perf_counter() - t_start

                pcm = (
                    (np.asarray(gen_result, dtype=np.float32).reshape(-1) * 32767.0)
                    .astype(np.int16)
                    .tobytes()
                )

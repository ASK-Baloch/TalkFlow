...
sys.path.insert(
    0, str(ROOT / "tools" / "cosyvoice-benchmark" / "scripts" / "final_benchmark")
)
from shared import SENTENCES, TRIALS, WARMUP_TEXT, get_vram_usage, validate_audio


def run_cosyvoice3(output_dir: Path, ref_audio: Path, transcript: str):
    print("Loading CosyVoice 3...")
    model_dir = (
        ROOT / "tools" / "cosyvoice-benchmark" / "models" / "Fun-CosyVoice3-0.5B-2512"
    )

    t0 = time.perf_counter()
    model = CosyVoice3(str(model_dir))
    ...
    if not ref_transcript.endswith("<|endofprompt|>"):
        ref_transcript += "<|endofprompt|>"
    ...
    for _ in model.inference_zero_shot(
        WARMUP_TEXT, ref_transcript, str(ref_audio), stream=False
    ):
        pass
    ...
    for sentence in SENTENCES:
        ...
        for trial in range(TRIALS):
            trial_data = run_trial(
                model, sentence, ref_transcript, ref_audio, trial + 1, 0
            )

            if trial_data["status"] in [
                "EMPTY_OUTPUT",
                "SILENT_OUTPUT",
                "CORRUPT_OUTPUT",
            ]:
                print(
                    f"Trial {trial + 1} failed with {trial_data['status']}. Retrying once..."
                )
                results["trials"].append(trial_data)  # Log failure
                trial_data = run_trial(
                    model, sentence, ref_transcript, ref_audio, trial + 1, 1
                )  # Retry
        ...
        if sentence_metrics:
            ...
            if ttfas:
                ...
                best_metric = min(
                    sentence_metrics, key=lambda x: abs(x["TTFA"] - median_ttfa)
                )
            else:
...
def run_trial(model, sentence, ref_transcript, ref_audio, trial_num, retry_num):
    ...
    try:
        generator = model.inference_zero_shot(
            sentence["text"], ref_transcript, str(ref_audio), stream=True
        )
        for chunk in generator:
            if ttfa is None:
                ttfa = (time.perf_counter() - t_start) * 1000.0
                first_chunk_size = (
                    len(chunk["tts_speech"])
            ...
            pcm_chunk = (
                (chunk_tensor.numpy().reshape(-1) * 32767.0).astype(np.int16).tobytes()
            )
...
if __name__ == "__main__":
    ...
    run_cosyvoice3(out_dir, ref_audio, transcript)

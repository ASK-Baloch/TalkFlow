...
def build_report(benchmark_dir):
    ...
    report_lines.append(
        f"| Model load (s) | {get_model_metric('Chatterbox Turbo', 'load_time')} | {get_model_metric('CosyVoice 3', 'load_time')} | {get_model_metric('CosyVoice 2', 'load_time')} |\n"
    )
    report_lines.append(
        f"| Warmup (s) | {get_model_metric('Chatterbox Turbo', 'warmup_time')} | {get_model_metric('CosyVoice 3', 'warmup_time')} | {get_model_metric('CosyVoice 2', 'warmup_time')} |\n"
    )
    report_lines.append(
        f"| Idle VRAM (MB) | {get_model_metric('Chatterbox Turbo', 'idle_vram')} | {get_model_metric('CosyVoice 3', 'idle_vram')} | {get_model_metric('CosyVoice 2', 'idle_vram')} |\n"
    )
    ...
    for t in trials:
        ...
        if t["status"] == "VALID":
            ...
            model_stats[m][f"cat_{t['category']}_ttfa"].append(
                t["TTFA"] if t["TTFA"] else t["generation_time"]
            )

        model_stats[m]["trials_total"] = model_stats[m].get("trials_total", 0) + 1
        if t["status"] == "VALID":
            model_stats[m]["valid"] = model_stats[m].get("valid", 0) + 1
        if t["status"] == "SILENT_OUTPUT":
            model_stats[m]["silent"] = model_stats[m].get("silent", 0) + 1
        if t["status"] == "EMPTY_OUTPUT":
            model_stats[m]["empty"] = model_stats[m].get("empty", 0) + 1
        if "ERROR" in t["status"]:
            model_stats[m]["error"] = model_stats[m].get("error", 0) + 1
        if t["status"] == "TIMEOUT":
            model_stats[m]["timeout"] = model_stats[m].get("timeout", 0) + 1
    ...
    report_lines.append(
        stat_row(
            "Peak VRAM (MB)", "vram_peak", lambda d: f"{np.max(d):.2f}" if d else "N/A"
        )
    )
    ...
    report_lines.append(
        stat_row("Number/Domain p50 Latency", "cat_NUMBER_DOMAIN_ttfa", p50)
    )
    ...
    for m in ["Chatterbox Turbo", "CosyVoice 3", "CosyVoice 2"]:
        ...
        report_lines.append(
            f"| {m} | {tot} | {val} | {sil} | {emp} | {err} | {oom} | {to} | {rate} |\n"
        )
    ...
    report_lines.append(
        "| VOICE CONSISTENCY | PENDING HUMAN LISTENING | PENDING HUMAN LISTENING | PENDING HUMAN LISTENING |\n"
    )
    report_lines.append(
        "| NATURALNESS | PENDING HUMAN LISTENING | PENDING HUMAN LISTENING | PENDING HUMAN LISTENING |\n"
    )

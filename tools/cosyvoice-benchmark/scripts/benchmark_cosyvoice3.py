...
def main():
    ...
    ref_transcript = (
        "hi how are you doing today great i am glad to hear that<|endofprompt|>"
    )
    ...
    results_dir = os.path.join(BENCHMARK_DIR, "results", "cosyvoice3")
    os.makedirs(results_dir, exist_ok=True)

    metrics = {"model": "Fun-CosyVoice3-0.5B-2512", "sentences": []}
    ...
    for j in cosyvoice.inference_zero_shot(
        "Hello there, I am just warming up the system to make sure it works.",
        ref_transcript,
        ref_audio_path,
        stream=False,
    ):
    ...
    for idx, text in enumerate(sentences):
        ...
        for iteration in range(10):
            ...
            generator = cosyvoice.inference_zero_shot(
                text, ref_transcript, ref_audio_path, stream=True
            )
            ...
            ttfa_ms = (
                (first_chunk_time - req_start) * 1000
                if first_chunk_time
                else (req_end - req_start) * 1000
            )
            ...
            rtf = (
                (total_time_ms / 1000.0) / (audio_duration_ms / 1000.0)

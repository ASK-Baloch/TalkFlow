...
class ChatterboxRuntime:
    ...
    async def start(
        ...
        reference = Path(self.settings.reference_audio)

        if not reference.exists():
            raise FileNotFoundError(
                f"Chatterbox reference audio not found: {reference}"
            )

        if self.settings.device == "cuda" and not torch.cuda.is_available():
            raise RuntimeError("CUDA requested for Chatterbox but CUDA is unavailable")
    ...
    async def synthesize(
        ...
        *,
        ...
        if len(text) > (self.settings.max_text_chars):
            raise ValueError("Text exceeds worker limit")

        async with self._lock:
            wav = await asyncio.to_thread(
                self.model.generate,
                text,
                audio_prompt_path=(self.settings.reference_audio),
            )

        audio = wav.detach().float().cpu().numpy().reshape(-1).astype(np.float32)

        source_rate = int(self.model.sr)

        if source_rate != self.settings.target_sample_rate:
            audio = soxr.resample(
                audio,
                source_rate,

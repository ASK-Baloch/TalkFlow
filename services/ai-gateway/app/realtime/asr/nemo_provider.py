...
class NemoStream(AsrStream):
    ...
    def get_partial(self) -> AsrDecodeResult:
        ...
        return self.provider.transcribe(
            audio, beam_size=self.beam_size, context_hints=self.context_hints
        )
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                or self._current_strategy != target_strategy
            ):
            ...
            logger.info(
                "Transcribe START: strategy=%s, beam=%s, config_hash=%s",
                self._current_strategy,
                self._current_beam_size,
                cfg_hash,
            )

            results = self.model.transcribe(
                audio=audio, return_hypotheses=(beam_size > 1)
            )

            logger.info(
                "Transcribe END: strategy=%s, beam=%s, config_hash=%s",
                self._current_strategy,
                self._current_beam_size,
                cfg_hash,
            )

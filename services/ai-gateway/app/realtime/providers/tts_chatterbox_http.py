...
class ChatterboxHttpTTSProvider(TTSProvider):
    ...
    def from_settings(
        ...
        return cls(
            base_url=(settings.tts_worker_url),
            timeout_seconds=(settings.tts_worker_timeout_seconds),
    ...
    async def stream(
        ...
        if self.client is None:
            raise RuntimeError("Chatterbox provider has not been started")

        if not request.text:
            raise ValueError("Dynamic TTS requires text")

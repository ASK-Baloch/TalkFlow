...
def test_normalization_positive():
    ...
    assert (
        normalize_transcript("I have Medicare part bee", context_hints=["ASK_PART_B"])
        == "I have Medicare Part B"
    )
    assert (
        normalize_transcript("I have Medicare part ay", context_hints=["ASK_PART_A"])
...
def test_normalization_negative():
    ...
    assert (
        normalize_transcript("The top of the pipe has good flow.")
        == "The top of the pipe has good flow."
    )
    ...
    assert (
        normalize_transcript("The top floor is closed.") == "The top floor is closed."
    )
    assert (
        normalize_transcript("The doctor is running low.")
        == "The doctor is running low."
    )
    assert normalize_transcript("I saw a bee.") == "I saw a bee."
    assert normalize_transcript("Say the letter A.") == "Say the letter A."
    assert (
        normalize_transcript("The blue crosswalk is ahead.")
        == "The blue crosswalk is ahead."
    )
...
def test_normalization_with_context():
    ...
    text, corrections = normalizer.normalize(
        "I want Medicare part ay.", context_hints=["ASK_PART_A"]
    )
...
def test_numbers():
    assert (
        normalize_transcript("My zip is seven five zero zero one.")
        == "My zip is 75001."
    )

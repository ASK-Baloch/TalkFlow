...
def test_dummy_tts_provider():
    async def run():
        provider = DummyTTSProvider.from_settings(Settings())

        await provider.start()

        chunks = []

        async for chunk in provider.stream(TTSRequest(text="hello")):
            chunks.append(chunk)
        ...
        assert all(len(chunk.pcm) == 320 for chunk in chunks)

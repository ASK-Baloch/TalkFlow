...
def run_benchmark():
    print(
        f"{'Words':<6} | {'Chars':<6} | {'TTFB (s)':<9} | {'Total (s)':<9} | {'Audio (s)':<9} | {'RTF':<6} | {'Peak VRAM':<10} | {'Peak CPU'}"
    )
    print("-" * 90)
    ...
    for text in UTTERANCES:
        ...
        data = json.dumps({"text": text, "voice_id": "talkflow_primary"}).encode(
            "utf-8"
        )
        ...
        try:
            ...
            print(
                f"{words:<6} | {chars:<6} | {ttfb:<9.3f} | {t_total:<9.3f} | {audio_duration:<9.3f} | {rtf:<6.3f} | {monitor.peak_vram:<6} MB   | {monitor.peak_cpu:>5.1f}%"
            )
